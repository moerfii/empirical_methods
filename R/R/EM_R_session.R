...
# what I usually do: session--set working directory--choose directory
getwd()
...
View(dplyr::select(berkeleyData,c('Admit','Total')))
### Remove dataset
rm(assetData)
...
# option 1
mrozData <-  mutate(mrozData, high_edu = factor((educ > 10), labels = c("low", "high")))
View(select(mrozData, c('educ','high_edu')))
mrozData <- mutate(mrozData, edu_dum = educ > 10)
View(select(mrozData, c('educ','edu_dum')))
# Option 2 (bit slower)
...
# select certain subsample(rows) based on condition
sub3 <- filter(mrozData, city==1)
sub3 <- subset(mrozData, city==1)
# of course can combine both and set more conditions
sub4 <- subset(mrozData, city==1 & educ>10, select = c("hours","age","wage","city", "educ"))

### deal with missing values
mrozData$highedu[mrozData$educ <= 10 & is.na(mrozData$educ) == FALSE] = "low"
mrozData$highedu[mrozData$educ > 10 & is.na(mrozData$educ) == FALSE] = "high"
# Important remarks:
   # -You can use == (exactly equal), >, <, <=, >=, or != (not equal)
   # -While "&" sets joint conditions, "|" means "or"
   # -You cannot say mrozData$educ!=NA to say "if educ is not missing"
   # -You need to use is.na(variable_name)==TRUE/FALSE, which reads: [if variable is (TRUE) or is not (FALSE) missing (na)]

### use pipe operator %>%: "then" operator, can combine several steps in one command, shortcut: command+shift+m fpr mac
...
str(berkeleyData) # summary of types of all variables

### basic summary statistics for one variable
mean(mrozData$wage)
sd(mrozData$wage)
summarise(mrozData, mean_wage = mean(wage))
summarise(mrozData, mean_wage = mean(wage, na.rm = TRUE), sd_wage = sd(wage), median_wage = median(wage), min_edu = min(educ))
sum1 <- summarise(mrozData, mean_wage = mean(wage, na.rm = TRUE), sd_wage = sd(wage), median_wage = median(wage), min_edu = min(educ))
sum(is.na(mrozData$wage)) # check how many missing values in the var

### summary frequency
table(mrozData$educ) # summary of frequency of categorical variable
table(mrozData$educ, mrozData$city) # summary of frequency by two categorical variables
...
# or just use group_by from dplyr
group <- group_by(mrozData, kidslt6)  # this doesn't change dataset itself, but changes how the dataset is perceived
summarise(group, mean_age = mean(age), max_hours = max(hours))

### export summary table
library(stargazer)
stargazer(mrozData, type="text")
stargazer(subset(mrozData, city==1, select = c(age, educ, wage, hours, exper)),type = "html", out = "./Figures&Tables/table1.html") # summary selected variables and export as html
...
ggplot(gapminder) +   # density by continent
...
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp), color = 'blue')
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp), size = 0.5)
...
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp, color = continent))
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp, size = continent))
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp, alpha = continent))
ggplot(gapminder) + geom_point(aes(x = gdpPercap, y = lifeExp, color = year >1980))
...
ols3 <- lm(lifeExp ~ log(gdpPercap) + pop + I(pop^2), data = gapminder)
...
### add fixed effects
library(fixest)
fe1 <- feols(lifeExp ~ gdpPercap + pop | continent, data = gapminder)
summary(fe1)
# this package is also easy for robust and cluster standard errors
fe2 <- feols(lifeExp ~ gdpPercap + pop | continent, se = 'hetero', data = gapminder)
summary(fe2)
fe3 <- feols(lifeExp ~ gdpPercap + pop | continent, cluster = c('country') , data = gapminder)
summary(fe3)
...
# simple t tests
t.test(ivdata$wage) # test if the mean is 0
...
# F tests
library(car)
linearHypothesis(ols,c("black = 0", "exper = 0"), type = c("F"))   # test if the two coefficients are jointly 0
